...
class TIN_ResNet50(nn.Module):
    def __init__(self):
        ...
        # self.resnet50 = torch.hub.load('pytorch/vision:v0.4.0', 'resnet50', pretrained=False)
    ...
    def forward(self, blobs, debug=False):
        ...
        # DEBUG
        # ======================================================================
        tmp_resnetv1 = resnetv1()
        tmp_resnetv1._init_head_tail(self.resnet50)

        data_l7   = h5py.File('./tmp/DEBUG/1.h5', 'r')
        data      = pickle.load(open('./tmp/DEBUG/img1-debug.pkl','rb'), encoding="bytes")
        data_conv1= pickle.load(open('./tmp/DEBUG/debug_conv.pkl','rb'), encoding="bytes")
        data_pad  = pickle.load(open('./tmp/DEBUG/debug_conv2d_same.pkl','rb'), encoding="bytes")
        data_fol  = pickle.load(open('./tmp/DEBUG/debug_follow.pkl','rb'), encoding="bytes")
        data_bin  = pickle.load(open('./tmp/DEBUG/debug_binary.pkl','rb'), encoding="bytes")
        tf_sp     = pickle.load(open('./tmp/DEBUG/debug_sp.pkl','rb'), encoding="bytes")

        tf_conv1  = data[0].transpose(0,3,1,2)
        tf_block1 = data[1].transpose(0,3,1,2)
        tf_head   = data[2].transpose(0,3,1,2)
        tf_H      = data[3].transpose(0,3,1,2)
        tf_O      = data[4].transpose(0,3,1,2)
        tf_7H     = data[5]
        tf_7O     = data[6]
        tf_afterConv2dSame = data_conv1[0].transpose(0,3,1,2) # 1,64,240,320
        tf_afterPadding    = data_conv1[1].transpose(0,3,1,2) # 1,64,242,322
        tf_afterPooling    = data_conv1[2].transpose(0,3,1,2) # 1,64,120,160
        tf_image           = data_pad[0].transpose(0,3,1,2) # 
        tf_image = torch.Tensor(tf_image).cuda()
        tf_imagePad        = data_pad[1].transpose(0,3,1,2) # 
        tf_fc9_binary      = data_bin[0]
        tf_cls_prob_binary = data_bin[1]
        tf_fc7SO           = data_l7['FSO'][4]
        ...
        (1, 600)
        '''
        tf_head_phi = data_fol[0].transpose(0,3,1,2)
        tf_head_g   = data_fol[1].transpose(0,3,1,2)
        tf_Att_H    = data_fol[2].transpose(0,3,1,2)
        tf_Att_O    = data_fol[3].transpose(0,3,1,2)
        tf_pool5_SH = data_fol[4].transpose(0,3,1,2)
        tf_pool5_SO = data_fol[5].transpose(0,3,1,2)
        tf_fc9_SH   = data_fol[6]
        tf_fc9_SO   = data_fol[7]
        tf_fc7_SHsp = data_fol[8]
        tf_cls_probH= data_fol[9]
        tf_cls_probO= data_fol[10]
        tf_cls_probsp= data_fol[11]
        
        H_boxes = O_boxes[0, 4].unsqueeze(0)
        O_boxes = O_boxes[0, 4].unsqueeze(0)
        spatial = torch.zeros(1, 1, 64, 64, 3).cuda()
        
        '''
        # test head
        test_head = self.image_to_head(image)   

        select_Box = O_boxes[0, 4]
        torch_H  = self._crop_pool_layer(test_head, select_Box.unsqueeze(0))

        fc7_H, fc7_O = self.res5(torch_H, torch_H)
        fc7_H = fc7_H.cpu().detach().numpy()
        print("tf_7H  :", np.mean(abs(fc7_H - tf_7H)))   # 5.682055e-07
        ...
        if debug:
            print("\n\n")
            # print("==> [in forward] self.gt_class_HO.shape:", self.gt_class_HO.shape)
            print("==> [in forward] spatial.shape:", spatial.shape)
            print("==> [in forward] H_boxes.shape:", H_boxes.shape) 
            print("==> [in forward] O_boxes.shape:", O_boxes.shape)  
        
        head = self.image_to_head(image)

        if debug:
            print("==> [in forward] image.shape:", image.shape) # [1, 3, 480, 640]
            print("==> [in forward] head.shape:", head.shape)   # [1, 1024, 30, 40]
        
        try:
            # using roi pooling
            # pool5_H = self.crop_pool_layer(head, H_boxes.squeeze(0))
            # pool5_O = self.crop_pool_layer(head, O_boxes.squeeze(0))

            # using built in crop_and_pool_layer
        ...
        if debug:
            print("==> [in forward] pool5_H.shape:", pool5_H.shape) # 20, 1024, 7, 7
            print("==> [in forward] pool5_O.shape:", pool5_O.shape)


        fc7_H, fc7_O = self.res5(pool5_H, pool5_O)
        
        if debug:
            print("==> [in forward] fc7_H.shape:", fc7_H.shape) # 20, 2048
            print("==> [in forward] fc7_O.shape:", fc7_O.shape)
        ...
        if debug:
            print("==> [in forward] head_phi.shape:", head_phi.shape) # 1, 512, 30, 40
            print("==> [in forward] head_g.shape:", head_g.shape)
        ...
        if debug:
            print("==> [in forward] Att_H.shape:", Att_H.shape)           # [512, 23, 40]
            print("==> [in forward] att_head_H.shape:", att_head_H.shape) # [1, 512, 23, 40]
        ...
        if debug:
            print("==> [in forward] Att_O.shape:", Att_O.shape)
            print("==> [in forward] att_head_O.shape:", att_head_O.shape)
        ...
        if debug:
            print("==> [in forward] pool5_SH.shape:", pool5_SH.shape) # [1, 1024, 27, 40]
            print("==> [in forward] pool5_SO.shape:", pool5_SO.shape) # [1, 1024, 27, 40] 
            print("==> [in forward] spatial[0,:,:,:,:2].shape:", spatial[0,:,:,:,:2].shape) # [1, 28, 64, 2, 3]
            # spatial[0].shape : N, 64, 64, 3
        ...
        if debug:
            print("==> [in forward] fc9_binary.shape:", fc9_binary.shape) # 
        # '''
        ...
        print("\n\n")
        print("fc7_SO     :", np.mean(abs(fc7_SO.cpu().detach().numpy() - tf_fc7SO)))
        print("fc7_H      :", np.mean(abs(fc7_H.cpu().detach().numpy() - tf_7H))) 
        print("fc7_O      :", np.mean(abs(fc7_O.cpu().detach().numpy() - tf_7O)))
        print("head_phi   :", np.mean(abs(head_phi.cpu().detach().numpy() - tf_head_phi)))
        print("head_g     :", np.mean(abs(head_g.cpu().detach().numpy()   - tf_head_g)))
        print("Att_H      :", np.mean(abs(Att_H.cpu().detach().numpy()    - tf_Att_H)))
        print("Att_O      :", np.mean(abs(Att_O.cpu().detach().numpy()    - tf_Att_O)))

        print("pool5_SH   :", np.mean(abs(pool5_SH.cpu().detach().numpy() - tf_pool5_SH)))
        print("pool5_SO   :", np.mean(abs(pool5_SO.cpu().detach().numpy() - tf_pool5_SO)))
        print("sp         :", np.mean(abs(sp.cpu().detach().numpy()       - tf_sp)))
        
        print("fc9_SH     :", np.mean(abs(fc9_SH.cpu().detach().numpy()   - tf_fc9_SH)))
        print("fc9_SO     :", np.mean(abs(fc9_SO.cpu().detach().numpy()   - tf_fc9_SO)))
        print("fc7_SHsp   :", np.mean(abs(fc7_SHsp.cpu().detach().numpy() - tf_fc7_SHsp)))

        print("cls_prob_H :", np.mean(abs(cls_prob_H.cpu().detach().numpy()   - tf_cls_probH)))
        print("cls_prob_O :", np.mean(abs(cls_prob_O.cpu().detach().numpy()   - tf_cls_probO)))
        print("cls_prob_sp:", np.mean(abs(cls_prob_sp.cpu().detach().numpy()  - tf_cls_probsp)))

        print("fc9_binary :", np.mean(abs(fc9_binary.cpu().detach().numpy()       - tf_fc9_binary)))
        print("clsProbBin :", np.mean(abs(cls_prob_binary.cpu().detach().numpy()  - tf_cls_prob_binary)))

        print(fc9_binary.cpu().detach().numpy(),  tf_fc9_binary)
        print(cls_prob_binary.cpu().detach().numpy(),  tf_cls_prob_binary)

        print("==> finsih forward")
        exit()
    ...
    def add_loss(self, debug=False):
        ...
        # binary_cross_entropy = torch.mean(nn.sigmoid_cross_entropy_with_logits(
        #     labels=label_binary, logits=cls_score_binary_with_weight))
        # print(cls_score_binary_with_weight.shape, label_binary.shape)
        ...
        # H_cross_entropy = torch.mean(nn.sigmoid_cross_entropy_with_logits(
        #     labels=label_HO[:self.H_num, :], logits=cls_score_H_with_weight[:self.H_num, :]))
        # O_cross_entropy = torch.mean(nn.sigmoid_cross_entropy_with_logits(
        #     labels=label_HO[:self.H_num, :], logits=cls_score_O_with_weight[:self.H_num, :]))  # fake :self.H_num
        # sp_cross_entropy = torch.mean(nn.sigmoid_cross_entropy_with_logits(
            # labels=label_HO, logits=cls_score_sp_with_weight))
