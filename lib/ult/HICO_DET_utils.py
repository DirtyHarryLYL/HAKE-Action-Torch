...
def get_map(keys, scores, bboxes):
    ...
    print('total    ap: %.4f, rec %.4f' % (float(np.mean(mAP)), float(np.mean(mrec))))
    print('rare     ap: %.4f, rec %.4f' % (float(np.mean(mAP[rare == 2])), float(np.mean(mrec[rare == 2]))))
    print('non-rare ap: %.4f, rec %.4f' % (float(np.mean(mAP[rare != 2])), float(np.mean(mrec[rare != 2]))))
