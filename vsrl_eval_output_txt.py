...
import numpy as np
import sys
sys.path.append('/Disk1/yonglu/iCAN/Data/v-coco/coco/PythonAPI')
...
class VCOCOeval(object):
  ...
  def _do_eval(self, detections_file, ovr_thresh=0.5):
    ...
    if type(detections_file) == type('string'):
        agent_ap = self._do_agent_eval(vcocodb, detections_file, ovr_thresh=ovr_thresh)
        scenario_1_ap = self._do_role_eval(vcocodb, detections_file, ovr_thresh=ovr_thresh, eval_type='scenario_1')
        scenario_2_ap = self._do_role_eval(vcocodb, detections_file, ovr_thresh=ovr_thresh, eval_type='scenario_2')
    
        return agent_ap, scenario_1_ap, scenario_2_ap
    else:
        scenario_1_ap = self._do_role_eval(vcocodb, detections_file, ovr_thresh=ovr_thresh, eval_type='scenario_1')
        return scenario_1_ap

  
  def _do_role_eval(self, vcocodb, detections_file, ovr_thresh=0.5, eval_type='scenario_1'):
    if type(detections_file) == type('string'):
        output_txt = detections_file[:len(detections_file)] + '.txt'
        with open(detections_file, 'rb') as f:
            dets = pickle.load(f)
    else:
        output_txt = None
        dets = detections_file
    ...
    if output_txt is None:
        ...
        for aid in range(self.num_actions):
          ...
          for rid in range(len(self.roles[aid])-1):
            info = '{: >23}: {:2d} {:d} AP = {:0.2f} (#pos = {:d})'.format(self.actions[aid]+'-'+self.roles[aid][rid+1], aid, rid, role_ap[aid, rid]*100.0, int(npos[aid]))
            print(info)
            f.write(info)
            f.write('\n')
        ...
        f.write('---------------------------------------------\n')
        f.close()
        return (np.nanmean(role_ap) * 25 - role_ap[-3][0]) / 24 * 100.00
  ...
  def _do_agent_eval(self, vcocodb, detections_file, ovr_thresh=0.5):
    ...
    if output_txt is None:
        ...
        f.write('---------------------------------------------\n')
        f.close()
        return np.nansum(agent_ap) * 100.00/self.num_actions
