...
List = load('/disk1/zhanke/TIN/Transferable-Interactiveness-Network/Data/hico_20160224_det/anno.mat', 'list_test');
List = List.list_test;
for i = 1 : 9658
    list(i) = str2double(List{i}(end-9 : end-4));
end

if ~isequal(sourceDir(length(sourceDir)), '/')
    sourceDir = strcat(sourceDir, '/');
end

new_80mat_dir = strcat(sourceDir, '80MAT_TRANSFORMED/');
mkdir(new_80mat_dir);

parpool(4)
parfor obj_idx = 1 : 80
    disp(obj_idx)
    Detection = load([strcat(sourceDir, 'detections_') sprintf('%02d',obj_idx) '.mat']);
...
%     clear all_boxes
    save_mat(obj_idx, new_80mat_dir, all_boxes)
end
