...
from hakeaction.models.DJRN_HICO import DJRN_ResNet50
...
def parse_args():
    ...
    parser.add_argument('--ckpt', 
            ...
            default="/home/zhanke/workspace/DJ-RN.torch/Weight/DJR/HOI_iter_400000.ckpt",
            type=str)
...
'''
    Loading weight for resnet_v1 50
    
    Note that in Batchnorm layer:
...
def DJRNLoadWeightFromTF(net, pretrained_dict):
    ...
    # attention_pool_layer_sp_network
    tmp_dict = {}
    tmp_dict['0.weight'] = torch.Tensor(pretrained_dict['Att_sp/fc1_b/weights'])
    tmp_dict['0.bias']   = torch.Tensor(pretrained_dict['Att_sp/fc1_b/biases'])
    net.attention_pool_layer_sp_network.load_state_dict(tmp_dict)

    # attention_3D
    #   fc3_G
    tmp_dict = {}
    tmp_dict['0.weight'] = torch.Tensor(pretrained_dict['attention_3D/fully_connected/weights'])
    tmp_dict['0.bias']   = torch.Tensor(pretrained_dict['attention_3D/fully_connected/biases'])
    net.fc3_G.load_state_dict(tmp_dict)
    #   A_3D
    tmp_dict = {}
    tmp_dict['weight'] = torch.Tensor(pretrained_dict['attention_3D/A_3D/weights'])
    tmp_dict['bias']   = torch.Tensor(pretrained_dict['attention_3D/A_3D/biases'])
    net.A_3D.load_state_dict(tmp_dict)
    ...
    # head_bottleneck_sp
    tmp_dict = {}
    tmp_dict['0.weight'] = torch.Tensor(pretrained_dict['bottleneck_sp/bottleneck_sp/weights'])
    tmp_dict['0.bias']   = torch.Tensor(pretrained_dict['bottleneck_sp/bottleneck_sp/biases'])
    net.head_bottleneck_sp.load_state_dict(tmp_dict)
    ...
    # body_to_head
    tmp_dict = {}
    tmp_dict['0.weight'] = torch.Tensor(pretrained_dict['body_to_head/fully_connected/weights'])
    tmp_dict['0.bias']   = torch.Tensor(pretrained_dict['body_to_head/fully_connected/biases'])
    tmp_dict['3.weight'] = torch.Tensor(pretrained_dict['body_to_head/fully_connected_1/weights'])
    tmp_dict['3.bias']   = torch.Tensor(pretrained_dict['body_to_head/fully_connected_1/biases'])
    net.body_to_head.load_state_dict(tmp_dict)
    
    # triplet_align.fc_sp
    tmp_dict = {}
    tmp_dict['0.weight'] = torch.Tensor(pretrained_dict['triplet_align/fc_sp/weights'])
    tmp_dict['0.bias']   = torch.Tensor(pretrained_dict['triplet_align/fc_sp/biases'])
    net.fc_sp.load_state_dict(tmp_dict)
    ...
    tmp_dict = {}
    tmp_dict['weight'] = torch.Tensor(pretrained_dict['space_classification/cls_score_D/weights'])
    tmp_dict['bias']   = torch.Tensor(pretrained_dict['space_classification/cls_score_D/biases'])
    net.cls_score_D.load_state_dict(tmp_dict)
    
    # joint_classification
    tmp_dict = {}
    tmp_dict['weight'] = torch.Tensor(pretrained_dict['joint_classification/cls_score_J/weights'])
    tmp_dict['bias']   = torch.Tensor(pretrained_dict['joint_classification/cls_score_J/biases'])
    net.cls_score_J.load_state_dict(tmp_dict)
...
if __name__ == "__main__":
    ...
    net = DJRNLoadWeightFromTF(net, pretrained_dict)
