...
class DetectionLayer(nn.Module):
    ...
    def forward(self, x, inp_dim, num_classes, confidence):
        x = x.data
        prediction = x.to(self.args.device)
        prediction = predict_transform(prediction, inp_dim, self.anchors, num_classes, confidence, self.args)
        return prediction
...
def create_modules(blocks, args):
...
#            upsample = Upsample(stride)
        ...
        #Yolo is the detection layer
            ...
            detection = DetectionLayer(anchors, args)
...
class Darknet(nn.Module):
    def __init__(self, cfgfile, args):
        ...
        self.net_info, self.module_list = create_modules(self.blocks, self.args)
